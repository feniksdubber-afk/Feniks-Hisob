...
# ==========================================
# 🚀 START VA TIZIMGA KIRISH (PIN TUZATILDI)
# ==========================================
...
def login(message):
    df = load_data()
    # TUZATISH: Barcha parollarni qat'iy ravishda matnga (string) aylantirib tekshiramiz
    parollar_royxati = df["Parol"].astype(str).tolist()
    
    if message.text in parollar_royxati:
...
# ==========================================
# 3. HISOB-KITOB (TO'LOV) - (+ VA - QO'SHILDI)
# ==========================================
...
def pay_init(message):
    ...
    for _, row in df.iterrows():
        if row["Ism"] != "Feniks":
            ...
            # TUZATISH: Endi qarz miqdoridan qat'iy nazar hamma aktyorlar ko'rinadi
            markup.add(types.InlineKeyboardButton(f"{row['Ism']} ({qarz:,})", callback_data=f"payto_{row['Ism']}"))
            buttons_added = True
            
    if buttons_added:
        bot.send_message(message.chat.id, "Hisob-kitob qilmoqchi bo'lgan aktyorni tanlang\n*(Musbat son qarzni kamaytiradi, Manfiy son (-) qarzni ko'paytiradi)*:", reply_markup=markup)
    else:
        bot.send_message(message.chat.id, "❌ Aktyorlar topilmadi.")

@bot.callback_query_handler(func=lambda call: call.data.startswith("payto_"))
def pay_amount(call):
    actor = call.data.split("_")[1]
    msg = bot.send_message(call.message.chat.id, f"💰 **{actor}** hisobiga qancha pul yozmoqchisiz?\n_(Masalan: 50000 yoki -50000)_")
    bot.register_next_step_handler(msg, pay_finalize, actor)

def pay_finalize(message, actor):
    try:
        # TUZATISH: Manfiy sonlarni ham bemalol qabul qiladi
        summa = int(message.text)
        df = load_data()
        df.loc[df["Ism"] == actor, "To'landi"] += summa
        save_df(df, DB_FILE)
        
        bot.send_message(ADMIN_ID, f"✅ O'zgarish saqlandi!\n👤 {actor}: {summa:,} so'm yozildi.")
        ...
        if actor_id != 0:
            try:
                if summa > 0:
                    bot.send_message(actor_id, f"💸 **To'lov kelib tushdi!**\nSizning hisobingizga {summa:,} so'm yozildi.")
                elif summa < 0:
                    bot.send_message(actor_id, f"📉 **Hisobingizdan ushlab qolindi:** {summa:,} so'm.")
            except: pass
    except:
        bot.send_message(ADMIN_ID, "⚠️ Xato! Faqat raqam kiriting (masalan: 50000 yoki -50000).")
...
# ==========================================
# 🎙 OVOZ TOPSHIRISH (AKTYOR FUNKSIYASI) - TEGILMADI
# ==========================================
user_audio_cache = {}
...
# ==========================================
@app.route('/')
def home(): return "Uzdubgo Boshqaruv Markazi ishlamoqda!"
